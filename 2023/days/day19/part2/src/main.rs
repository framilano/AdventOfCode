...
fn main() {
    ...
    let (workflows, _parts): (HashMap<String, Vec<String>>, Vec<Part>) = parse_input(&arr_str);

    //The idea should be generating for each workflow a vector of conditions in AND that bring

    let mut accepted_counter: usize = 0;

    for i in 1..=4000 {
        for j in 1..=4000 {
            for k in 1..=4000 {
                for l in 1..=4000 {
                    if is_accepted(&Part { x: i, m: j, a: k, s: l }, &workflows, "in") {
                        accepted_counter += 1;
                        println!("{}", accepted_counter);
                    }
                }
            }
        }
    ...
    println!("Sum of accepted parts: {}", accepted_counter);
}
