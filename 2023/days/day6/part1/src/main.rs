...
fn extract_time_distance(time_str: &str, distance_str: &str) -> Vec<(u32, u32)> {
    let mut time_dist_vect:Vec<(u32, u32)> = Vec::new();
    ...
    for (time, distance) in zip(times, distances) {
        time_dist_vect.push((time.parse::<u32>().unwrap(), distance.parse::<u32>().unwrap()));
    }

    return time_dist_vect;
}

fn get_number_of_ways(time_dist_tuple: (u32, u32)) -> u32 {
...
fn main() {
    ...
    let time_dist_vec: Vec<(u32, u32)> = extract_time_distance(arr_str[0], arr_str[1]);

    let mut product_of_ways:u32 = 1;
