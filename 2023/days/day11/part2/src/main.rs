...
/**
 * The shortest path in a discrete environment like this one without diagonals
 * is simply the sum between the row diff and the column diff, of course
 * both taken as absolute values.
 * 
 * We can't represent in this matrix the "expanding" universe thing, due to the 
 * gigantic size the matrix would take, so for each empty row and empty column
 * between the two nodes we simply add how many rows and columns this expanded universe
 * has. So in part1 it just added 1 more row per each empty row, same for columns.
 * In part2 it adds 999999 rows and columns per each empty row and column
 */
fn find_shortest_path(couple: (Node, Node), empty_rows: &Vec<i32>, empty_columns: &Vec<i32>) -> u64 {
    ...
    //We can assume safely that couple.0 is always on top
    ...
    //Adding how many empty columns there are
    //We can't safely assume that couple.0 is always on the left,
    //so we first need to find which node is on the left
...
//Returns a list of empty rows indexes inside the matrix
...
//Returns a list of empty columns indexes inside the matrix
