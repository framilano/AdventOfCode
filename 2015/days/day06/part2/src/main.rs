...
fn update_lights_matrix(line: &str, lights_matrix: &mut [[i32; 1000]; 1000]) {
    ...
    if line.contains("turn on") {
        ...
        for index in coords[0][0]..=coords[1][0] {
            for jndex in coords[0][1]..=coords[1][1] {
                lights_matrix[index as usize][jndex as usize] += 2;
            }
        }
    }
}
...
fn main() {
    ...
    let mut lights_matrix: [[i32; 1000]; 1000] = [[0; 1000]; 1000];

    for line in arr {
        update_lights_matrix(line, &mut lights_matrix);
    }

    let mut lights_on_total_value = 0;
    for i in 0..1000 {
        for j in 0..1000 {
            lights_on_total_value += lights_matrix[i][j]
        }
    }

    println!("Total brightness value: {}", lights_on_total_value);
}
